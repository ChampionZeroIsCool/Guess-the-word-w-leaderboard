        ...
        if (!guessedLetters.includes(letter)) {
            ...
            if (currentWord.includes(letter)) {
                ...
                if (allLettersGuessed) {
                    const updatedWins = props.wins + 1;
                    const updatedTotalGames = props.wins + props.losses + 1;
                    props.setWins(updatedWins);
                    props.setTotalGames(updatedTotalGames);
                    props.setWinsPercentage(Math.round((updatedWins / updatedTotalGames) * 100));
                    props.setLossesPercentage(100 - Math.round((updatedWins / updatedTotalGames) * 100));
                ...
                if (remainingGuesses <= 0) {
                    const updatedLosses = props.losses + 1;
                    const updatedTotalGames = props.wins + props.losses + 1;
                    props.setLosses(updatedLosses);
                    props.setTotalGames(updatedTotalGames);
                    props.setLossesPercentage(Math.round((updatedLosses / updatedTotalGames) * 100));
                    props.setWinsPercentage(100 - Math.round((updatedLosses / updatedTotalGames) * 100));
                    props.setHasUnsavedChanges(false)
                    setGameResult('loss')
                    setShowSummary(true)
                }
    ...
    return (
        <div className='text-gray-900 text-2xl'>
            ...
            <div className='gap-1 flex pb-8 items-center'>
                ...
                <select disabled={showSummary} onChange={(e) => handleDropdownChange(Number(e.target.value))} value={currentWordLength} id='wordLength' name='wordLength' className='cursor-pointer'>
            ...
            <div>
                ...
                <p>Stats:</p>
                <p>Wins: <b>{props.wins}</b> {props.totalGames > 0 && `(${props.winsPercentage}%)`}</p>
                <p>Losses: <b>{props.losses}</b> {props.totalGames > 0 && `(${props.lossesPercentage}%)`}</p>
                <p>Total games: <b>{props.totalGames}</b></p>
                <button onClick={props.handleReset} className='cursor-pointer border-2 border-black p-2.5 px-3 mt-2 rounded-xl group hover:font-bold flex gap-2 items-center'>Reset
                    <ArrowPathIcon className='h-6 w-6 stroke-2 group-hover:stroke-4' />
                </button>
            </div>
        </div>
